...
export default class ClientHotelArticleList extends Component {
  ...
  deleteArticle(id) {
    axios.delete(`http://localhost:9000/articlesToRoom/${id}`).then(() => {
      this.setState((prevState) => ({
  ...
  getAllArticlesRoom() {
    axios
      .get("http://localhost:9000/articlesToRoom")
      .then((response) => {
        this.setState({
      ...
      >
        <table className="w-full text-center">
          <thead className="border-b-2 border-blue-800 text-sm text-blue-800">
            <tr>
              <th className="text-start py-2">Article</th>
              <th className="text-center py-2 pr-20">Nombre</th>
              <th className="text-start pr-8 py-2">Facturation</th>
              <th className="text-center pr-8 py-2"></th>
            </tr>
          </thead>
          <tbody className="text-gray-600 text-sm font-medium">
            {this.state.articlestoRoom.map((elm, index) => (
              <tr key={index} className="border-b border-gray-300">
                ...
                <td className=" py-2 ">
                  <div className="border shadow-sm w-8 py-1 rounded text-center">
                    <FontAwesomeIcon
                      onClick={() => {
